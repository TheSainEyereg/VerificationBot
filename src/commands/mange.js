			...
			interaction.editReply({
				embeds: [
					success(null, "Ник изменен!", `Новое имя игрока ${member.toString()} сохранено! Старый никнейм также сохранен и доступен через команду /info.`, {embed: true})
						.setFooter({ text: !settings/settings.serverless ? "Учтите, что старый ник был удален из вайтлиста, зайти на сервер с ним больше невозможно" : "Бот находится в serverless режиме и не может управлять вайтлистом, учтите это!" })
				]
			});
		}
	}
